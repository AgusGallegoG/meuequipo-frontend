...
import { defaultSponsorTable, type SponsorTable } from '@/sponsor/domain/SponsorTable';

export const useSponsorAdminStore = defineStore('sponsorAdmin', {
  state: () => {
    return {
      data: {
  ...
  getters: {
    ...
    getEditionSponsor: (state) => state.data.selectedToEdit,
  ...
  actions: {
    setTableData(data: SponsorTable) {
    ...
    setSelectedToEdit(user: Sponsor) {
